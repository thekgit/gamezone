  ...
  try {
    ...
    if (!secret) {
      return NextResponse.json(
        { error: "Missing EXIT_QR_SECRET in env" },
        { status: 500 }
      );
    }
    ...
    // Optional: block QR for already-ended sessions (matches old behavior)
    ...
    // ✅ IMPORTANT: new random token every click (so new QR every time)
    const tokenPayload = {
      sid: session_id,
