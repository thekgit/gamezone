...
// ✅ IMPORTANT: This makes UI compatible with BOTH old API + new API
function normalizeRow(r: AnyRow): Row {
  return {
    id: r.id,

    // new API uses created_at, old API sometimes uses timestamp
    created_at: r.created_at ?? r.timestamp ?? null,

    // new API uses full_name/phone/email, old API uses name/phone/email
    full_name: r.full_name ?? r.name ?? r.visitor_name ?? null,
    phone: r.phone ?? r.visitor_phone ?? null,
    email: r.email ?? r.visitor_email ?? null,

    // new API uses game_name, old API uses game
    game_name: r.game_name ?? r.game ?? null,

    // players exists in sessions table (if missing, show null)
    players:
      typeof r.players === "number" ? r.players : r.players != null ? Number(r.players) : null,

    // new API uses slot_start/slot_end, old API uses start_time/end_time
    slot_start: r.slot_start ?? r.start_time ?? r.started_at ?? null,
    slot_end: r.slot_end ?? r.end_time ?? r.ends_at ?? null,

    status: r.status ?? null,

    // exit_time should be QR scan time (ended_at)
    exit_time: r.exit_time ?? r.ended_at ?? null,
  ...
  const genQr = async (r: Row) => {
    ...
    try {
      const res = await fetch("/api/admin/exit-code", {
        ...
        // ✅ keep backward-compatible: send session_id
    ...
    <div className="text-white">
      ...
      <div className="mt-6 overflow-auto rounded-xl border border-white/10 bg-[#0b0b0b]">
        <table className="w-full text-sm">
          ...
          <tbody>
            {rows.map((r) => {
              const completed = !!r.exit_time; // ✅ only QR scan completes
