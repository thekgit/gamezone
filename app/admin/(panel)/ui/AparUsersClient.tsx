...
export default function AparUsersClient() {
  ...
  const importUsers = async () => {
    ...
    try {
      ...
      setMsg(
        `✅ Imported: ${data?.created ?? 0}, Existing kept: ${data?.skipped_existing ?? 0}, Invalid: ${
  ...
  const createManual = async () => {
    ...
    try {
      ...
      setMsg("✅ User created (or already existed). Default password: NEW12345 (only for new users).");
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-2">
        <div className="rounded-xl border border-white/10 bg-black/30 p-4">
          ...
          <button
            ...
            className="mt-3 w-full rounded-xl bg-blue-600 py-2.5 font-semibold hover:bg-blue-500 disabled:opacity-40 disabled:cursor-not-allowed"
          >
