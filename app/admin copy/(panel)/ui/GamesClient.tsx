  ...
  const createGame = async () => {
    ...
    const res = await fetch("/api/admin/games", {
      ...
      body: JSON.stringify({
        ...
        capacity_per_slot: 1, // ✅ required by your DB NOT NULL
      }),
    });
  ...
  const saveEdit = async () => {
    ...
    const res = await fetch(`/api/admin/games`, {
      ...
      body: JSON.stringify({
        ...
        // keep capacity_per_slot stable unless you want to edit it later:
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
          ...
          <div className="text-xs text-white/50 mb-4">
            Creates a game used in bookings.
          </div>
        ...
        {/* Discount UI placeholder (as you had) */}
        <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
          <div className="font-semibold">Discount Value</div>
          <div className="text-xs text-white/50 mb-4">
            UI only (you said you’ll edit later).
          </div>
          ...
          <button
            onClick={() => setMsg("Discount UI is ready (not stored yet).")}
        ...
        <div className="p-4 border-b border-white/10 flex items-center justify-between">
          <div>
            ...
            <div className="text-xs text-white/50">
              Status column shows slot occupancy (green = booked).
            </div>
          </div>
          <button
            onClick={load}
              ...
              {activeGames.map((g) => {
                ...
                return (
                  <tr key={g.id} className="border-t border-white/10 hover:bg-white/5">
                    ...
                    {/* ✅ STATUS = slot buttons (non-clickable) */}
                    <td className="p-3">
                      {total > 0 ? (
